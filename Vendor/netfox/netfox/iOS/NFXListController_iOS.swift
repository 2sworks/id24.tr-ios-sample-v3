...
class NFXListController_iOS: NFXListController, UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating, UISearchControllerDelegate, DataCleaner {
    ...
    override func viewDidLoad() {
        ...
        let rightButtons = [
            ...
            UIBarButtonItem(barButtonSystemItem: .action, target: self, action: #selector(NFXListController_iOS.shareButtonPressed)),
    ...
    /// Listedeki istekleri dosya olarak dışa aktarır. Arama ve süzgeçler
    /// uygulandıktan sonraki liste aktarılır.
    ...
    private func export(format: NFXExportFormat) {
        guard let url = NFXLogExporter.exportRequests(models: tableData, format: format) else { return }
